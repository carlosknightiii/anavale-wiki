...
function initAppearanceListeners() {
  ...
  if (skinTone) skinTone.addEventListener('input', function() {
    updateAIPrompt();
    updateBodyFigure();
    updateFaceFigure();
  });
...
// ── FACE FIGURE SVG ─────────────────────────────────────────────
function updateFaceFigure() {
  var faceShape  = getVal('app-face-shape');
  var eyeColor   = getVal('app-eye-color');
  var eyeShape   = getVal('app-eye-shape');
  var facialHair = getVal('app-facial-hair');
  var hairColor  = getVal('app-hair-color');
  var hairStyle  = getVal('app-hair-style');
  var skinTone   = document.getElementById('app-skin-tone') ? document.getElementById('app-skin-tone').value : '#c68642';
  var markings   = [];
  document.querySelectorAll('input[name="app-markings"]:checked').forEach(function(cb) { markings.push(cb.value); });

  // ── Skin tone propagation ──────────────────────────────────
  var skinEls = ['face-outline','face-neck','face-ear-left','face-ear-right'];
  skinEls.forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.setAttribute('fill', skinTone);
  });

  // ── Face shape → ellipse rx/ry ─────────────────────────────
  var faceEl = document.getElementById('face-outline');
  if (faceEl) {
    var faceShapes = {
      'round':        [52, 58],
      'oval':         [44, 66],
    ...
    var fs = faceShapes[faceShape] || [50, 62];
    faceEl.setAttribute('rx', fs[0]);
    faceEl.setAttribute('ry', fs[1]);
  }
  ...
  var eyeColorMap = {
    ...
    'gold':            '#C8940A',
    'silver':          '#A0A8B0',
    'completely black': '#111111',
    'completely white': '#F0F0F0',
    'mismatched':      null
  };
  var eyeFill = eyeColorMap[eyeColor] || '#3A5A8A';
  var eyeL = document.getElementById('eye-left');
  var eyeR = document.getElementById('eye-right');
  if (eyeL) eyeL.setAttribute('fill', eyeColor === 'mismatched' ? '#3A5A8A' : eyeFill);
  if (eyeR) eyeR.setAttribute('fill', eyeColor === 'mismatched' ? '#6B3A2A' : eyeFill);
  ...
  var es = eyeShapeMap[eyeShape] || [9, 6];
  [eyeL, eyeR].forEach(function(el) {
    if (el) { el.setAttribute('rx', es[0]); el.setAttribute('ry', es[1]); }
  });
  ...
  var hairColorMap = {
    ...
    'auburn':            '#6a2c10',
    'red':               '#8a2010',
    'strawberry blonde': '#b06040',
    'blonde':            '#c8a040',
    'platinum blonde':   '#e0d4b0',
    'grey':              '#808080',
    'white':             '#e8e4e0',
    'silver':            '#b0b8c0',
    'blue-black':        '#181828',
  ...
  var hFill = hairColorMap[hairColor] || '#3a2010';
  ['hair-top','hair-sides','hair-sides-r','hair-long'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.setAttribute('fill', hFill);
  });

  // ── Hair style → show/hide hair elements ───────────────────
  var hairTop    = document.getElementById('hair-top');
  var hairSidesL = document.getElementById('hair-sides');
  var hairSidesR = document.getElementById('hair-sides-r');
  var hairLong   = document.getElementById('hair-long');
  var shortStyles = ['cropped','short','shaved on sides'];
  var medStyles   = ['medium-length','tightly coiled','tied back'];
  var longStyles  = ['long straight','long wavy','long curly','braided','locs','wild and untamed'];
  var noHairStyle = hairColor === 'no hair';

  if (hairTop)    hairTop.setAttribute('opacity',    noHairStyle ? '0' : '1');
  if (hairSidesL) hairSidesL.setAttribute('opacity', (!noHairStyle && (medStyles.indexOf(hairStyle) >= 0 || longStyles.indexOf(hairStyle) >= 0)) ? '1' : '0');
  if (hairSidesR) hairSidesR.setAttribute('opacity', (!noHairStyle && (medStyles.indexOf(hairStyle) >= 0 || longStyles.indexOf(hairStyle) >= 0)) ? '1' : '0');
  if (hairLong)   hairLong.setAttribute('opacity',   (!noHairStyle && longStyles.indexOf(hairStyle) >= 0) ? '1' : '0');

  // ── Facial hair ────────────────────────────────────────────
  var fhEl = document.getElementById('facial-hair-shape');
  if (fhEl) {
    var fhPaths = {
      'light stubble': { d: 'M88,162 Q100,168 112,162 Q112,166 100,167 Q88,166 88,162Z', op: '0.5' },
      'a short beard':  { d: 'M85,162 Q100,176 115,162 Q115,174 100,176 Q85,174 85,162Z', op: '1' },
      'a full beard':   { d: 'M82,160 Q100,180 118,160 Q120,182 100,185 Q80,182 82,160Z', op: '1' },
      'a long beard':   { d: 'M82,158 Q100,182 118,158 Q122,195 100,200 Q78,195 82,158Z', op: '1' },
      'a moustache':    { d: 'M88,158 Q100,164 112,158 Q110,162 100,163 Q90,162 88,158Z', op: '1' },
      'sideburns':      { d: 'M72,136 Q70,148 72,158 Q78,160 80,156 Q78,146 76,136Z M120,136 Q122,148 120,158 Q114,160 112,156 Q114,146 116,136Z', op: '1' }
    };
    var fhData = fhPaths[facialHair];
    if (fhData) {
      fhEl.setAttribute('d', fhData.d);
      fhEl.setAttribute('fill', hFill !== 'none' ? hFill : '#3a2010');
      fhEl.setAttribute('opacity', fhData.op);
    } else {
      fhEl.setAttribute('opacity', '0');
    }
  }

  // ── Facial markings ────────────────────────────────────────
  var markingMap = {
    'freckles':                  'marking-freckles',
    'a scar':                    'marking-scar',
    'tattoos':                   'marking-tattoo'
  };
  Object.keys(markingMap).forEach(function(key) {
    var el = document.getElementById(markingMap[key]);
    if (el) el.setAttribute('opacity', markings.indexOf(key) >= 0 ? '1' : '0');
  });
}

// ── BODY FIGURE SVG ─────────────────────────────────────────────
function updateBodyFigure() {
  var height   = getVal('app-height');
  var build    = getVal('app-build');
  var age      = getVal('app-age');
  var skinTone = document.getElementById('app-skin-tone') ? document.getElementById('app-skin-tone').value : '#c68642';

  // ── Skin tone on all body parts ────────────────────────────
  var bodyParts = ['body-head','body-neck','body-torso','body-arm-left','body-arm-right','body-leg-left','body-leg-right'];
  ...
  var scaleY = heightScaleMap[height] || 1.0;
  ...
  var scaleX = buildScaleMap[build] || 1.0;

  var group = document.getElementById('body-figure-group');
  if (group) group.setAttribute('transform', 'translate(80,150) scale(' + scaleX + ',' + scaleY + ')');

  // ── Age lines opacity ──────────────────────────────────────
  var ageOpMap = { 'young': 0, 'young adult': 0.1, 'adult': 0.2, 'middle-aged': 0.4, 'elder': 0.7 };
  var ageEl = document.getElementById('age-lines');
  if (ageEl) ageEl.setAttribute('opacity', ageOpMap[age] !== undefined ? ageOpMap[age] : 0);

  // ── Height label ───────────────────────────────────────────
  var labelEl = document.getElementById('height-label');
  if (labelEl) labelEl.textContent = height || '';
}
