...
*Session Aug 9 2026 — Character Sheet Tab Redesign v1, slices 1–2: tab shell + shared MainInfo header, then Ability Scores tab. See Decision Log for full detail.*
- `sheet/index.html` now has a 4-button tab bar (Your Story / Ability Scores / Inventory / Class) with working show/hide. Ability Scores now holds its real content (6 ability cards, tooltips, no feats); Your Story holds everything else that already existed; Inventory and Class are still "coming soon" placeholders. Not yet built: Your Story's own internal reorganization per the tab spec's card table, Class tab content, Inventory tab (waiting on its Figma pass).
...
- Next: Class tab (Overview mapping is a known quantity; Key Features needs the `FEAT_ICONS`/name-level parser ported from `character.js`), then Your Story reorg, then Inventory once its Figma pass lands. See docs build spec for full order.
