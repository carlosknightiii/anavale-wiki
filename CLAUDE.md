...
- Two build-spec inaccuracies found and NOT silently worked around: (1) spec claims Class tab's Key Features parsing logic "already" exists in the character creator "via the same pattern" — it doesn't; `character.js`'s `renderClassGrid()` only assigns an icon and renders each raw string whole, it never splits name/description/level. Only the icon table was a real port. (2) spec says there are "66 features... 7 of the 66" bare subclass entries — actual count is 72 (12 classes × 6) with 8 bare entries; the parser handles all of them correctly regardless, this is just the spec's own count being wrong, not a code issue. (3) spec says Class tab's spell slots section is "existing `character_sheet.spell_slots`/`spell_slots_max` UI" that just needs relocating — confirmed via full-codebase grep that no such UI exists anywhere (`sheet/index.html`, `dm.html`, `character.html`, `js/character.js` all come up empty), even though both Supabase columns exist. Not built this pass — there is nothing to relocate; building it from scratch would be new design work outside "no redesign yet."
...
- Next: Your Story reorg, then Inventory once its Figma pass lands. Spell slots UI still needs a product decision (build fresh vs. defer) before it can go on the Class tab. See docs build spec for full order.
