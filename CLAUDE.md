...
*Session Aug 9 2026 — Character Sheet Tab Redesign v1, slices 1–3: tab shell + shared MainInfo header, Ability Scores tab, Class tab. See Decision Log for full detail.*
- `sheet/index.html` now has a 4-button tab bar (Your Story / Ability Scores / Inventory / Class) with working show/hide. Ability Scores and Class both hold real content now; Your Story holds everything else that already existed; Inventory is still a "coming soon" placeholder (waiting on its Figma pass). Not yet built: Your Story's own internal reorganization per the tab spec's card table.
- Class tab: class summary card + Hit Die/Primary/Saves/Casting/Armor/Tools/Weapons (Hit Die is a new row — wasn't shown anywhere before; `?` tooltips added on Hit Die/Saves/Weapons only, matching `character.html`'s pattern exactly) + Key Features (icon + name/description/level-badge, `FEAT_ICONS` ported verbatim from `character.js`, but the name/description/level parser is genuinely new code — verified against all 72 real feature strings in Supabase `classes.features` before writing, not assumed from the build spec). Alternating row shading is pure CSS `:nth-child(even)` using a new `--blue-500` token (`#28304a`, pulled from the real Figma file via `get_variable_defs` on node `52:2705`, not guessed).
- Spell Slots stopgap added to the Class tab (DM-approved, explicitly NOT the real Figma redesign): current/max pips per spell level 1–9, wired to `character_sheet.spell_slots`/`spell_slots_max` (both real jsonb columns, `{}` on all 3 characters — no prior UI ever wrote to them, confirmed by grep). Shape is `{level: count}`, invented here since nothing existed to match. Clicking a filled pip spends it (unfills that pip and everything after), clicking an empty one restores up to it — same interaction as the death-save pips. Kept deliberately minimal per the DM's instruction — no per-class slot tables, no auto-calculated max, just a manual current/max editor.
- Three build-spec inaccuracies found and NOT silently worked around: (1) spec claims Class tab's Key Features parsing logic "already" exists in the character creator "via the same pattern" — it doesn't; `character.js`'s `renderClassGrid()` only assigns an icon and renders each raw string whole, it never splits name/description/level. Only the icon table was a real port. (2) spec says there are "66 features... 7 of the 66" bare subclass entries — actual count is 72 (12 classes × 6) with 8 bare entries; the parser handles all of them correctly regardless, this is just the spec's own count being wrong, not a code issue. (3) spec described Class tab's spell slots section as "existing `character_sheet.spell_slots`/`spell_slots_max` UI" that just needed relocating — confirmed via full-codebase grep that no such UI ever existed anywhere. Built fresh as a minimal stopgap per DM direction rather than left undone (see above).
- The Tab Redesign v1 build spec doc itself isn't a file in this repo (checked `docs/` — only `character-sheet-redesign-handoff.md` from Aug 7 and the unrelated project-plan doc exist). It lives in project knowledge outside git, so the corrections above were handed back to the DM to apply there rather than edited directly.
- `hp_temp` investigation (DM asked to rule out a code bug after it read 5 unprompted on Kael mid-session): traced every write path — only `setTempHP()` in `sheet/index.html` ever writes `hp_temp`, gated on an explicit "Set" click with `parseInt(input.value)`, no default/fallback that could produce 5 on its own. `js/hp-calc.js`'s `reconcileHp()`/`computeHpBase()` never reference `hp_temp` at all (grep confirms). Reviewed this session's own interactions and never touched that control. Conclusion: no write-path bug found; production Supabase is shared regardless of which URL serves the HTML (local dev server and the deployed site hit the same DB), so most likely explanation is the DM or Kael's player testing independently on the live site. Not reverted — no basis to call it wrong.
- New shared MainInfo header (renders once above the tabs): species paragraph (new, from `species.description`), Speaks block (new, joins the new `languages` table), Conditions +Add/remove (new), Inspiration toggle (new), relocated AC badge/HP bar/appearance paragraph (same logic, new location), new Temp HP badge (hides at 0/null), new death-save pip tracker that swaps in for the HP bar at 0 HP.
- Ability Scores tab: added `?` tooltip icons on each ability card (`ABILITY_TIPS`, verbatim from `character.html`'s richer copy — replaces nothing, sheet never had these before) via a new `wireAllTooltips()` sweep (`document.querySelectorAll('[data-tip]')`) instead of one-off manual wiring per element; feats moved off this tab onto the Background card in Your Story's Overview accordion (`.cs-feat-list` reused as-is, just a new render location) per the spec's explicit instruction.
- Found and fixed two real bugs during required browser testing (neither in the original spec): (1) the death-save view had no way to heal back above 0 HP — added a minimal heal input alongside the pips, healing above 0 now also resets both pip counts per the 5e rule; (2) same session, confirmed no equivalent gap exists in the Ability Scores tooltip sweep (idempotent via `_tipWired`, safe to call from multiple render paths).
- Confirmed `js/hp-calc.js` (untracked going into this session) is real, load-bearing code — a shared HP-calc module referenced by both `dm.html` and `sheet/index.html`, from a prior session that ended without a commit or Decision Log entry. Committed along with `dm.html`'s matching `reconcileHp()` integration in the SCC player bar, since the two files don't function correctly without it.
- Known duplication, pre-existing not introduced this session: when a character has `appearance_prompt` set, it already restates the same structured fields (height/build/skin/eyes/hair) the Appearance-section chips show, and a third retelling exists in the Overview summary card's "Your Appearance" paragraph. Relocating the paragraph into the header increased the *distance* between paragraph and chips — same redundant content, now less visually connected. Flagged for a decision during the real Your Story tab build, not fixed here.
- Two more pre-existing uncommitted items found, unrelated to this session, not touched: `docs/character-sheet-redesign-handoff.md` (Aug 7 session output, still untracked) and `.claude/launch.json` (Aug 7 Decision Log says it was "added" but it was never actually committed). `.claude/settings.local.json` is also untracked and should stay that way (local-only) — repo has no `.gitignore` entry for `.claude/` yet.
- Next: Your Story reorg, then Inventory once its Figma pass lands. See docs build spec for full order.
