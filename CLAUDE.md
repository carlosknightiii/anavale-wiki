...
*Session Aug 9 2026 — Character Sheet Tab Redesign v1, first slice: tab shell + shared MainInfo header. See Decision Log for full detail.*
- `sheet/index.html` now has a 4-button tab bar (Your Story / Ability Scores / Inventory / Class) with working show/hide — only Your Story holds real content this pass, the other three are "coming soon" placeholders. Not yet built: moving Ability Scores/Inventory/Class content into their own tabs (next up), Your Story's own internal reorganization per the tab spec's card table.
...
- Found and fixed a real bug during testing (not in the original spec): the death-save view had no way to heal back above 0 HP — added a minimal heal input alongside the pips; healing above 0 now also resets both pip counts, per the 5e rule.
- Confirmed `js/hp-calc.js` (untracked going into this session) is real, load-bearing code — a shared HP-calc module referenced by both `dm.html` and `sheet/index.html`, from a prior session that ended without a commit or Decision Log entry. Committed tonight along with `dm.html`'s matching `reconcileHp()` integration in the SCC player bar, since the two files don't function correctly without it.
- Known duplication, pre-existing not introduced tonight: when a character has `appearance_prompt` set, it already restates the same structured fields (height/build/skin/eyes/hair) the Appearance-section chips show, and a third retelling exists in the Overview summary card's "Your Appearance" paragraph. Relocating the paragraph into the header (this session) increased the *distance* between paragraph and chips — same redundant content, now less visually connected. Flagged for a decision during the real Your Story tab build, not fixed here.
- Two more pre-existing uncommitted items found, unrelated to tonight, not touched: `docs/character-sheet-redesign-handoff.md` (Aug 7 session output, still untracked) and `.claude/launch.json` (Aug 7 Decision Log says it was "added" but it was never actually committed). `.claude/settings.local.json` is also untracked and should stay that way (local-only) — repo has no `.gitignore` entry for `.claude/` yet.
- Next: Ability Scores tab (real content + tooltip generalization), then Class, then Your Story reorg, then Inventory once its Figma pass lands. See docs build spec for full order.
